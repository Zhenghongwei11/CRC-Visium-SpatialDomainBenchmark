...
- `GSE280318`: CRC Visium, 10x `filtered_feature_bc_matrix.h5`-style inputs (used for baseline-only local pilot in this version).
...
- `GSE280318` is available locally and is included in baseline smoke tests, but BayesSpace ingestion in this repository currently expects the flat matrix format. As a result, BayesSpace benchmarking and claim-gated comparisons are restricted to datasets with the supported flat format.
...
- Cross-dataset harmonization is constrained by input file formats; extending BayesSpace coverage to `GSE280318` requires a validated ingestion path that preserves comparability.
