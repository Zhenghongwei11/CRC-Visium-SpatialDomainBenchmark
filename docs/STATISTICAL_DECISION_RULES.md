...
- 2026-09-07: Added the reviewer-requested cross-method boundary-reliability analysis before inspecting its new outputs. The fixed protocol is `docs/BOUNDARY_RELIABILITY_PROTOCOL.md`.
- 2026-09-08: Added a descriptive interval-support classification after the cross-method results were audited. The original opposite-sign point-estimate results remain unchanged. The added tables identify settings in which the reference and at least one opposite-sign alternative both have spot-bootstrap intervals excluding zero. This is not a new confirmatory gate and does not resolve spatial dependence.
- 2026-09-09: Added targeted official-implementation sensitivity summaries and a stability-stratified downstream summary. These are descriptive final-revision checks and do not alter the locked claim gates.
