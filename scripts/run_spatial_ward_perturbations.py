#!/usr/bin/env python3
"""Export fixed-K spatial Ward partitions under declared graph perturbations."""
        ...
        for entry in entries:
            ...
            for k_value in k_values:
                for neighbor_count in neighbor_values:
                    graph = benchmark.spatial_connectivity_graph(coords, neighbors=int(neighbor_count))
                    ...
                    if observed != int(k_value):
                        raise RuntimeError(
                            f"Spatial Ward produced {observed} domains for {sample_id}, "
                            f"K={k_value}, neighbors={neighbor_count}"
                        )
                        ...
                        writer.writerow(
                            {
                                ...
                                "status": "success",
                                "notes": "fixed-K spatial Ward graph-specification sensitivity",
                            }
                        )
                    handle.flush()
                    print(
                        f"[ward] dataset={args.dataset_id} sample={sample_id} "
                        f"K={k_value} neighbors={neighbor_count}",
                        flush=True,
                    )
